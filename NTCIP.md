...
| 訊息類別     | 設備日期、時間管理                                                                       |
| 目的         | 對時誤差回報及要求中央中心對時。                                                         |
| 用途         | 1. 現場設備系統時間誤差管理。<br>2. 用於現場設備收到對時訊息(0F H+12 H)。                |
| 訊息格式     | 0F H+92 H+SecDif                                                                         |
| 訊息參數定義 | SecDif：1 Byte，時間誤差（秒），整數(00 &#126;128)。<br>*時間誤差超過127秒以上時均設SecDif=128(第7位元=On)表示誤差過大。<br>*定時對時時為整點對時，則於該換之時間年、月、日、時、分誤差不回應。<br>*SecDif值為0時，表示終端設備要求中央中心對時。 |
| 訊息處理步驟 | <table><tr><td>控制中心</td><td>←</td><td>號誌控制器、車輛偵測器、資訊可變標誌</td></tr><tr><td></td><td colspan="2">1. 對時誤差超過3秒時，上傳訊息0F H+92 H及時間誤差值SecDif。<br>2. 接收0F H+92 H及時間誤差SecDif。</td></tr></table> |
| 參考訊息     | 0F H+12 H、0F H+C2 H、0F H+42 H、0F H+02 H。                                             |

## 訊息說明：0F H+13 H 設備韌體燒錄日期及版本管理 設定
