...
在「都市交通控制通訊協定」3.0版中之起始碼，則編為2碼之複合碼，即在原指令碼前加設備碼，變為2碼；第1碼為設備碼，第2碼為指令碼，各碼長度均為1 Byte。

### 二、訊息起始碼編排原則及定義

1. **「都市地區交通控制通訊協定」3.0 版訊息起始碼編排方式**

(1) **第 1 碼：設備碼**

運用原部頒 87 年版通訊協定之各設備區段的最後一個 XF H（其中 X 可為 0~F，如下所示）為第 1 碼，E0 H~EF H 給新增設備；F0~FF H 保留。
...
編訂方式為 (A)(B) 相對，(C)(D) 相對，且 (A)(B)(C)(D) 有功能相依性，即若設定訊息為 1X H，其相對設定回報指定為 9X H，且若其查詢訊息為 4X H，則其相對查詢回報訊息為 CX H。X 為指某功能之低位元編號 (0~F)。
